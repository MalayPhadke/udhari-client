...
export default function AddUdhariWS(props) {
    ...
    const MoneyReceived = (e) => {
        ...
        props.addStatusHandler(prevState => !prevState)
            Axios.put(`${config.SERVER_URI}/${props.username}/udhari_to_pay`, {username, contact, email, upi_id, amount})
                .then(res => {     
    ...
    const MoneyPayed = (e) => {
        ...
        props.addStatusHandler(prevState => !prevState)
            Axios.put(`${config.SERVER_URI}/${props.username}/udhari_to_get`, {username, contact, email, upi_id, amount})
            .then(res => {
                if(typeof res.data === 'object' && res.data !== null){
                        // const {status, amount} = res.data[0].udhari;
                        // const {name, upi_id, _id} = res.data[0];
    ...
    const findUser = (username) => {
        Axios.get(`${config.SERVER_URI}/${username}`)
            .then(res => {
                if(res.data){
                    // alert("user found");
